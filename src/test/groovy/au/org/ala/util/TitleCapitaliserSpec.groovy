...
class TitleCapitaliserSpec extends Specification {
    ...
    def 'test list capitalisation 1'() {
        expect:
        capitaliser.capitalise(['the', 'mill', 'on', 'the', 'Floss']) == 'The Mill on the Floss'
    }

    def 'test list capitalisation end 1'() {
        expect:
        capitaliser.capitalise(['A', 'one', 'a', 'two', 'A']) == 'A One a Two A'
    }

    def 'test enumeration capitalisation 1'() {
        expect:
        capitaliser.capitalise(Collections.enumeration(['the', 'mill', 'on', 'the', 'Floss'])) == 'The Mill on the Floss'
    }

    def 'test enumeration capitalisation end 1'() {
        expect:
        capitaliser.capitalise(Collections.enumeration(['A', 'one', 'a', 'two', 'A'])) == 'A One a Two A'
    }
